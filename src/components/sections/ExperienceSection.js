    ...
    <section id="experience" className="py-20 md:py-28">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        <SectionHeader
          ...
          subtitle="Production engagements, freelance projects, and full-stack work — ordered by recency."
        />
                ...
                <div className="surface rounded-2xl p-6">
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                    ...
                    <div className="flex flex-col items-end text-xs">
                      <span className="font-medium text-zinc-700">{job.period}</span>
                      <span className="text-zinc-500 mt-0.5">{job.type}</span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-2 mb-4">
                    {job.achievements.map((line, idx) => (
                      <li key={idx} className="flex items-start gap-2.5 text-sm text-zinc-700 leading-relaxed">
                        <Check size={14} className="flex-shrink-0 mt-0.5 text-indigo-500" strokeWidth={2.5} />
                        <span>{line}</span>
                      </li>
                    ))}
                  </ul>
